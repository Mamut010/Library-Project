<x-layouts.default-layout title="Book Delete Confirmation" selected="Admin">
    ...
    <x-slot:content>
        <div id="bordermother">
            <a href="{{ route('booktypes.index') }}" target="_self" title="Back">
                <img class="back-button" src="https://www.seekpng.com/png/detail/87-875958_back-arrow-in-circle-symbol.png">
            </a><br>
            <h1 class="manage-heading">Delete Book Type</h1>
            <div class="warning">
                <b>
                    This action cannot be undone.<br>Are you sure about this action?
                </b>
            </div>
            <form action="{{ route('booktypes.destroy', ['booktype' => $booktype]) }}" method="POST">
                @method('DELETE')
                @csrf
                <input type="submit" value="Delete Book Type" onclick="alert('The book type has been deleted!\nReturning to the bookshelf...')">
            </form>
        </div>
    </x-slot:content>
